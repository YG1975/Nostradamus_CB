...
A running ODC deployment is essentially the `datacube-core` software connected to a PostgreSQL/PostGIS database, with data indexed into it via product definitions and dataset documents. Once populated, the cube can be queried through the Python API, browsed through the Explorer, or served via OGC web services, all reading from the same underlying index. The [**Installation**](dc_installation.qmd){target="_blank"} and [**Quick Start**](dc_quickstart.qmd){target="_blank"} pages walk through setting this up step by step.
